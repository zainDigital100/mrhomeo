      ...
      <div className="container mx-auto px-4">
        <nav className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-xl font-display font-semibold text-foreground">My Homeo<span className="text-primary">MyHomeo
Home
Diseases
AI Doctor
About
Start Consultation</span>
          ...
          <div className="hidden md:flex items-center gap-1 bg-muted/50 rounded-full p-1">
            ...
            return <Link key={item.path} to={item.path}>
                  <Button variant={isActive ? "nav-active" : "nav"} size="sm" className="rounded-full px-4">
      ...
      <AnimatePresence>
        ...
        height: 0
            ...
            return <Link key={item.path} to={item.path} onClick={() => setMobileMenuOpen(false)}>
                    <div className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${isActive ? "bg-primary text-primary-foreground" : "hover:bg-accent"}`}>
                      <Icon className="w-5 h-5" />
                      <span className="font-medium">{item.label}</span>
                    </div>
                  </Link>;
          })}
              <Link to="/ai-treatment" onClick={() => setMobileMenuOpen(false)} className="block mt-4">
